...
document.getElementById('searchButton').addEventListener('click', () => {
  ...
  fetch('https://studentbackend-mq3j.onrender.com/getResults', {
    ...
    .then(studentData => {
      ...
      const studentDataArray = [cleanKeys(studentData)];
      renderTable(studentDataArray);
      calculateStatusAndSGPA(studentDataArray);
    })
    .catch(error => {
      console.error('Error fetching data:', error);
      dataContainer.innerHTML = `<p>Failed to fetch data: ${error.message}. Please try again later.</p>`;
    });
});
